...
XPortalNetworks includes lightweight, privacy-first anonymous telemetry to help monitor mod stability, diagnose unhandled errors, and understand active version adoption across game updates.
...
* **Full Player Control**: Telemetry is completely optional and can be enabled or disabled at any time:
  ...
  * **In Configuration**: Set `Enable Anonymous Telemetry = false` under `[Local Config]` in `BepInEx/config/vapok.mods.xportalnetworks.cfg` (or adjust it live via the BepInEx Configuration Manager).
